    ...
    public class WebSocketChannelBase : IDisposable
    {
        ...
        protected async void StartDisconnectionMonitor()
        {
            ...
            catch (Exception e)
            {
                ...
                // one of the send/receive exception types and we won't receive any closing
                // message from client.
                if (!IsSendReceiveException(e))
                {
                    Trace.TraceError("Unexpected exception encountered while monitoring websocket channel for disconnections:\n{0}", e);
                    throw;
                }
            }
        }
        ...
        private async void OnDisconnectionCheckTimerTick(object sender, EventArgs args)
        {
            ...
            catch (Exception e)
            {
                if (!IsSendReceiveException(e))
                {
                    Trace.TraceError("Unexpected exception encountered while checking for disconnections in websocket channel:\n{0}", e);
                    throw;
                }
            }
        }
