...
private:
    ...
    static const WORD       BinsForFFT = 512;

    // Buffer used to store audio stream data for FFT calculation
    float                   m_audioInputForFFT[BinsForFFT];
    ...
    XDSP::XVECTOR *         m_binsFFTRealStorage;
    XDSP::XVECTOR *         m_binsFFTImaginaryStorage;
    float *                 m_binsFFTReal;
    float *                 m_binsFFTImaginary;
    ...
    float					m_binsFFTDisplay [BinsForFFT / 2];

    // Number of audio samples accumulated so far
    int                     m_accumulatedSampleCount;
...
private:
    ...
    static const int        AudioSamplesPerEnergySample = 40;

    // Number of energy samples that will be visible in display at any given time.
    static const int        EnergySamplesToDisplay = 780;

    // Buffer used to store audio stream energy data as we read audio
    float                   m_energyBuffer[EnergySamplesToDisplay];

    // Buffer used to store audio stream energy data ready to be displayed
    float                   m_energyDisplayBuffer[EnergySamplesToDisplay];

    // Sum of squares of audio samples being accumulated to compute the next energy value
    float                   m_accumulatedSquareSum;
