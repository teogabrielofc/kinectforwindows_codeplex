...
CAudioExplorer::~CAudioExplorer()
{
    ...
    for (int i = 0; i < CountOfVisualizers; ++i)
    {
...
{
    {0, IDS_AES_OFF, 0, true},
    {1, IDS_AES_ON, 2, false}
};
...
LRESULT CALLBACK CAudioExplorer::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        case WM_INITDIALOG:
        {
            ...
            HRESULT hr = m_pAudioPanel->Initialize(GetDlgItem(m_hWnd, IDC_AUDIOVIEW), m_pD2DFactory, Display);
            if (FAILED(hr))
            {
            ...
            // Set the timer so we get periodic notications to update the displays
...
HRESULT CAudioExplorer::InitializeAudioSource()
{
    ...
    memcpy (&m_wfxOut, &fmt, sizeof(WAVEFORMATEX));
    DMO_MEDIA_TYPE mt = {0};
    ...
    memcpy(mt.pbFormat, &m_wfxOut, sizeof(WAVEFORMATEX));
...
void CAudioExplorer::ProcessAudio()
{
    ...
    do
    {
        ...
        if (cbProduced > 0)
        {
            ...
            for (int iVisualizer = 0; iVisualizer < CountOfVisualizers; ++iVisualizer)
            {
