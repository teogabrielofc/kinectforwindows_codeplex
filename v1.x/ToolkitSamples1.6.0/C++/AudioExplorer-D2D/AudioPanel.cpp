...
HRESULT AudioPanel::Initialize(const HWND hWnd, ID2D1Factory* pD2DFactory, const UINT energyToDisplay)
{
    ...
    m_displayWidth = energyToDisplay;
    m_displayHeight = m_displayWidth / 4;
...
HRESULT AudioPanel::CreateDisplay()
{
    ...
    D2D1_SIZE_U size = D2D1::SizeU(m_displayWidth, m_displayHeight);
    HRESULT hr = S_OK;
    ...
    if (SUCCEEDED(hr))
    {
        m_pDisplay->CopyFromMemory(NULL, m_pBackground, m_backgroundStride);
    }
