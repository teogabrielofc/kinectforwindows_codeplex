...
AudioPanel::AudioPanel() : 
    ...
    m_energyDisplayWidth(0),
    m_energyDisplayHeight(0),
    m_pEnergyBackground(NULL),
    m_energyBackgroundStride(0),
    m_pEnergyForeground(NULL),
    m_energyForegroundStride(0),
...
HRESULT AudioPanel::Initialize(const HWND hWnd, ID2D1Factory* pD2DFactory, const UINT energyToDisplay)
{
    ...
    m_energyDisplayWidth = energyToDisplay;
    m_energyDisplayHeight = m_energyDisplayWidth / 4;
...
void AudioPanel::UpdateEnergy(const float *pEnergy, const UINT energyLength)
{
    ...
    m_pEnergyDisplay->CopyFromMemory(NULL, m_pEnergyBackground, m_energyBackgroundStride);
    ...
    for (UINT i = 0; i < min(energyLength, m_energyDisplayWidth); ++i)
    {
        const int cHalfImageHeight = m_energyDisplayHeight / 2;

        // Each bar has a minimum height of 1 (to get a steady signal down the middle) and a maximum height
        // equal to the bitmap height.
        int barHeight = static_cast<int>(max(1.0f, (pEnergy[i] * m_energyDisplayHeight)));
        ...
        m_pEnergyDisplay->CopyFromMemory(&barRect, m_pEnergyForeground, m_energyForegroundStride);
    }
}
...
HRESULT AudioPanel::CreateEnergyDisplay()
{
    ...
    D2D1_SIZE_U size = D2D1::SizeU(m_energyDisplayWidth, m_energyDisplayHeight);
    HRESULT hr = S_OK;

    // Allocate background and set to white
    m_energyBackgroundStride = cBytesPerPixel * m_energyDisplayWidth;
    int numBackgroundBytes = m_energyBackgroundStride * m_energyDisplayHeight;
    ...
    m_energyForegroundStride = cBytesPerPixel;
    m_pEnergyForeground = new BYTE[cBytesPerPixel * m_energyDisplayHeight];
    UINT *pPixels = reinterpret_cast<UINT*>(m_pEnergyForeground);
    for (UINT iPixel = 0; iPixel < m_energyDisplayHeight; ++iPixel)
    {
